...
This document lists all template tokens used in this Civo + GitHub cloud provider configuration. These tokens are replaced during the GitOps template instantiation process.
...
|-------|---------------|-------------|
| `CLOUD_PROVIDER` | `<CLOUD_PROVIDER>` | Cloud provider name (civo) |
| `CLOUD_REGION` | `<CLOUD_REGION>` | Civo region for deployment |
| `NODE_COUNT` | `<NODE_COUNT>` | Number of worker nodes |
| `NODE_TYPE` | `<NODE_TYPE>` | Civo instance size for nodes |

### Civo-Specific Configuration

| Token | Template Value | Description |
|-------|---------------|-------------|
| `WORKLOAD_CIVO_ACCOUNT_NAME` | `<WORKLOAD_CIVO_ACCOUNT_NAME>` | Civo account name for workload clusters |
...
| `CLUSTER_NAME` | `<CLUSTER_NAME>` | Civo Kubernetes cluster name |
...
| `EXTERNAL_DNS_PROVIDER_NAME` | `<EXTERNAL_DNS_PROVIDER_NAME>` | DNS provider (civo, cloudflare, etc.) |
...
- Civo provides cost-effective Kubernetes clusters with fast provisioning
- GPU cluster templates available for AI/ML workloads