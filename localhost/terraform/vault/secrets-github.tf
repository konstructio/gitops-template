...
resource "vault_generic_secret" "atlantis_secrets" {
  ...
  data_json = jsonencode(
  {
	...
	ARGOCD_SERVER = "http://localhost:8080", # todo is this port correct
	ARGO_SERVER_URL = "argo.argo.svc.cluster.local:443", # todo is this port correct
	...
	TF_VAR_email_address = "<EMAIL_ADDRESS>",
	TF_VAR_github_token = var.github_token,
	TF_VAR_kubefirst_bot_ssh_public_key = var.kubefirst_bot_ssh_public_key,
	TF_VAR_vault_addr = "http://vault.vault.svc.cluster.local:443",
	TF_VAR_vault_token = "k1_local_vault_token",
	VAULT_ADDR = "http://vault.vault.svc.cluster.local:443",
