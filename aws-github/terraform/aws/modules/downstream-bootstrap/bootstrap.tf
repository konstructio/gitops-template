...
data "aws_ssm_parameter" "external_secrets_operator" {
  name = "secret/atlantis"
}

provider "kubernetes" {
  host                   = data.aws_ssm_parameter.cluster.data["host"]
  cluster_ca_certificate = base64decode(data.aws_ssm_parameter.cluster.data["cluster_ca_certificate"])
