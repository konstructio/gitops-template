...
This document lists all template tokens used in this Google Cloud + GitHub cloud provider configuration. These tokens are replaced during the GitOps template instantiation process.
...
|-------|---------------|-------------|
| `CLOUD_PROVIDER` | `<CLOUD_PROVIDER>` | Cloud provider name (google) |
| `CLOUD_REGION` | `<CLOUD_REGION>` | Google Cloud region for deployment |
| `NODE_COUNT` | `<NODE_COUNT>` | Number of worker nodes |
| `NODE_TYPE` | `<NODE_TYPE>` | GCE machine type for nodes |

### Google Cloud-Specific Configuration

| Token | Template Value | Description |
|-------|---------------|-------------|
| `GOOGLE_PROJECT` | `<GOOGLE_PROJECT>` | Google Cloud project ID |
| `GOOGLE_UNIQUENESS` | `<GOOGLE_UNIQUENESS>` | Unique identifier for Google resources |
...
| `CLUSTER_NAME` | `<CLUSTER_NAME>` | GKE cluster name |
...
| `EXTERNAL_DNS_PROVIDER_NAME` | `<EXTERNAL_DNS_PROVIDER_NAME>` | DNS provider (google, cloudflare, etc.) |
...
| `KUBEFIRST_ARTIFACTS_BUCKET` | `<KUBEFIRST_ARTIFACTS_BUCKET>` | GCS bucket for artifacts |
| `KUBEFIRST_STATE_STORE_BUCKET` | `<KUBEFIRST_STATE_STORE_BUCKET>` | GCS bucket for Terraform state |
| `KUBEFIRST_STATE_STORE_BUCKET_HOSTNAME` | `<KUBEFIRST_STATE_STORE_BUCKET_HOSTNAME>` | Hostname for state bucket |
| `VAULT_DATA_BUCKET` | `<VAULT_DATA_BUCKET>` | GCS bucket for Vault data |
...
| `WORKLOAD_NODE_TYPE` | `<WORKLOAD_NODE_TYPE>` | Machine type for workload nodes |
...
- Google Cloud uses GCS buckets for object storage
- GKE provides managed Kubernetes with Google Cloud integration